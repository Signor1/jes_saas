...
export default function StoreDetailPage() {
  ...
  const { getStores,  updateStore, loading, error } = useAPI();
  const { id } = params
  ...
  useEffect(() => {
    ...
    const fetchStore = async() => {
      ...
      const thisStore = storeData.find((eachStore: any) => eachStore.id === id)
        ...
        <div className="flex flex-col">
          <header className="flex h-14 items-center gap-4 border-b bg-muted/40 px-4 lg:px-6">
            ...
            <div className="ml-auto flex items-center gap-2">
              <ShareStore storeName={store?.name as string} storeSlug={store?.id as string}>
